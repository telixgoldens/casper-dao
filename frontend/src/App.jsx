...
import { deployVote } from './utils/casperService'; // 👈 IMPORTED SERVICE
...
function App() {
  ...
  const [isVoting, setIsVoting] = useState(false); // 👈 ADDED STATE

  // 👇 ADDED VOTE HANDLER
  const handleVote = async (choice) => {
    if (!activeKey) return alert("Connect Wallet first!");
    
    if (!confirm(`Vote ${choice ? "YES" : "NO"}? This will cost ~150 CSPR.`)) return;
  ...
  if (!isConnected) {
    ...
    const mainBg = activeTab === 'create' ? 'bg-nebula bg-grid-texture' : 'bg-slate-900';

    return (
        <div className={`min-h-screen ${mainBg} text-white font-sans selection:bg-teal-500 selection:text-black`}>
    ...
    <main className="max-w-6xl mx-auto p-8  pt-3 container">
        ...
        <div className="flex gap-6 border-b border-gray-800 mb-8">
            <button 
                ...
                className={`pb-2 px-2 nav-active ${activeTab === 'create' ? 'text-teal-400 border-b-2 border-teal-400' : 'text-gray-500 hover:text-gray-300'}`}
            >
                Create DAO
            </button>
            <button 
                onClick={() => setActiveTab('vote')}
                className={`pb-2 px-2 nav-active ${activeTab === 'vote' ? 'text-teal-400 border-b-2 border-teal-400' : 'text-gray-500 hover:text-gray-300'}`}
            >
                Active Proposals
            </button>
        </div>
                    ...
                    <div className="bg-slate-800 p-8 rounded-xl border border-gray-700 relative">
                        ...
                        {isVoting && (
                            <div className="absolute inset-0 bg-slate-900/80 backdrop-blur-sm z-10 flex items-center justify-center rounded-xl">
                                <div className="text-teal-400 font-bold animate-pulse">Processing Vote...</div>
                            </div>
                        )}
