...
export const CasperProvider = ({ children }) => {
    ...
    try {
      ...
      if (typeof provider === "function") {
        try {
          ...
          console.log(
            "Provider appears to be a factory function — used returned object for methods."
          );
        } catch (callErr) {
          console.warn(
            "Provider is a function but calling it failed:",
            callErr
          );
        }
      }

      const keys = Object.keys(prov || {}).length
        ? Object.keys(prov)
        : Object.getOwnPropertyNames(prov || {});
      ...
      if (prov && typeof prov.requestConnection === "function") {
        ...
        try {
          const res = await prov.request({
            method: "casper_requestConnection",
          });
          connected = !!res;
        } catch (e) {
          try {
        ...
        console.warn(
          "No known connection method on provider. Provider keys:",
          keys
        );
      }
  ...
  const disconnectWallet = async () => {
    ...
    if (provider) {
      try {
        ...
        console.warn("Wallet refused to disconnect via API (ignoring):", err);
      }
    }
    setActiveKey(null);
      ...
      try {
        ...
        if (typeof provider.isConnected === "function") {
          ...
          try {
            ...
            console.log(
              "Not connected (no isConnected and getActivePublicKey failed)."
            );
            connected = false;
          }
        }
  ...
  return (
    <CasperContext.Provider
      value={{ activeKey, isConnected, connectWallet, disconnectWallet }}
    >
      {children}
    </CasperContext.Provider>
  );
